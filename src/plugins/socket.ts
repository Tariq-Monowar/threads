...
export default fp(async (fastify) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join", async (userId: string) => {
      ...
      if (pendingCall && pendingCall.status === "calling") {
        const callerId = pendingCall.with;
        const callType = pendingCall.type;

        try {
          // Fetch caller info from database
          const callerIdNumber = Number(callerId);
          if (!Number.isNaN(callerIdNumber)) {
            const callerData = await prisma.user.findUnique({
              where: { id: callerIdNumber },
              select: {
                id: true,
                name: true,
                avatar: true,
            ...
            if (callerData) {
              const callerInfo = {
                id: callerData.id,
                name: callerData.name || `User ${callerId}`,
                avatar: FileService.avatarUrl(callerData.avatar || ""),
              };

              // Send call_incoming event to the newly online user
              ...
              fastify.log.info(
                `Pending call delivered to ${userId} from ${callerId}`
              );
            }
          }
        } catch (error: any) {
          fastify.log.error(
            `Failed to fetch caller info for pending call: ${error.message}`
          );
        }
      }
    ...
    socket.on(
      ...
      async ({
        ...
        // Send push notification to receiver (via FCM tokens) - works for both online and offline users
        ...
        // Send socket event only if receiver is currently online
        ...
        if (receiverSocketId) {
          ...
          fastify.log.info(`Call incoming event sent via socket to ${receiverId}`);
        } else {
          // Receiver is offline - push notification already sent above
          fastify.log.info(
            `Receiver ${receiverId} is offline, push notification sent. Call will be delivered when user comes online.`
          );
        }
