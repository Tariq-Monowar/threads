...
exports.getSingleConversation = exports.getMyConversationsList = void 0;
...
const getMyConversationsList = async (request, reply) => {
    try {
        ...
        const processConversationMembers = (members, isGroup, currentUserId) => {
            ...
            if (!isGroup) {
                return formattedMembers.filter((member) => member.userId !== currentUserId);
            }
            // For group conversations, show current user first, then others (max 3)
            const [currentUserMembers, otherMembers] = formattedMembers.reduce(([current, rest], member) => member.userId === currentUserId
                ? [[...current, member], rest]
                : [current, [...rest, member]], [[], []]);
         ...
         */
        const batchCountUnreadMessages = async (prisma, conversationIds, currentUserId) => {
         ...
         */
        const transformConversation = async (conversation, currentUserId, unreadCount, prisma) => {
            ...
            const currentUserMember = conversation.members.find((member) => member.userId === currentUserId);
            ...
            return {
                ...
                messages: conversation.messages.map((message) => (0, message_utils_1.transformMessage)(message, participantIds)),
                avatar: conversation.avatar
                    ? (0, baseurl_1.getImageUrl)(conversation.avatar)
                    : null,
                unreadCount,
         ...
         */
        const buildConversationWhereClause = (currentUserId) => {
            return {
                members: {
                    some: {
        ...
        const { page, limit, lastMessageLimit, skip } = parsePaginationParams(request.query);
        ...
        const transformedConversations = await Promise.all(conversations.map(async (conversation) => transformConversation(conversation, currentUserId, unreadCountsMap[conversation.id] || 0, prisma)));
...
exports.getMyConversationsList = getMyConversationsList;
const getSingleConversation = async (request, reply) => {
    try {
        ...
        const messageLimit = Math.min(Math.max(parseInt(request.query?.message) || 50, 1), 100);
        // ── FETCH CONVERSATION ───────────────────────────────────────────────
        ...
        const participantIds = getParticipantIds(conversation.members);
        const unreadCount = await countUnreadMessages(prisma, conversationId, currentUserId);
        // Check if blocked
        const isBlocked = await checkIfBlocked(prisma, conversation, currentUserId);
        // Get blockBy list
        const getBlockByList = async (prisma, conversation, currentUserId) => {
        ...
        const transformedConversation = {
            ...
            messages: conversation.messages
                ...
                .map((message) => (0, message_utils_1.transformMessage)(message, participantIds)),
...
//# sourceMappingURL=conversation.controllers.js.map