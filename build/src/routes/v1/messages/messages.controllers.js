...
exports.getMessages = exports.markMessageAsDelivered = exports.markMultipleMessagesAsRead = exports.updateMessage = exports.deleteMessageForEveryone = exports.deleteMessageForMe = exports.sendMessage = exports.deleteMessage = void 0;
...
const deleteMessage = async (request, reply) => {
    try {
        ...
        try {
            ...
            if (filenames.length) {
                fileService_1.FileService.removeFiles(filenames);
            }
        }
        catch (_) { }
        return reply.send({
...
const sendMessage = async (request, reply) => {
    try {
        ...
        const transactionResult = await prisma.$transaction(async (tx) => {
            ...
            const filesCreate = files.length > 0
                ? files.map((file) => ({
                    ...
                    fileExtension: path_1.default.extname(file.originalname || "").replace(".", "") || null,
                }))
                : [];
        ...
        const transformedMessage = (0, message_utils_1.transformMessage)(messageForResponse, participantIds);
        const response = {
            success: true,
        ...
        setImmediate(async () => {
            try {
                ...
                for (const member of transactionResult.members) {
                    ...
                    const fcmTokens = (0, jsonArray_1.getJsonArray)(member.user?.fcmToken, []);
                    ...
                    const validTokens = fcmTokens.filter((token) => typeof token === 'string' && token.trim().length > 0);
                    ...
                    for (const token of validTokens) {
                        ...
                        pushPromises.push(request.server.sendDataPush(token, pushData)
                            .then((result) => {
                            if (result.success) {
        ...
        try {
            ...
            if (uploadedFilenames.length) {
                fileService_1.FileService.removeFiles(uploadedFilenames);
            }
        }
        catch (_) { }
        request.log.error(error);
...
const deleteMessageForMe = async (request, reply) => {
    try {
        const { messageId } = request.params;
        const { myId } = request.body;
        const prisma = request.server.prisma;
        ...
        const deletedUsers = (0, jsonArray_1.getJsonArray)(message.deletedForUsers);
        if ((0, jsonArray_1.jsonArrayContains)(message.deletedForUsers, myIdInt)) {
            return reply.status(400).send({
                success: false,
        ...
        await prisma.message.update({
            where: { id: messageId },
            data: {
                deletedForUsers: (0, jsonArray_1.jsonArrayAdd)(message.deletedForUsers, myIdInt),
            },
        });
        return reply.send({
            success: true,
...
const deleteMessageForEveryone = async (request, reply) => {
    try {
        const { messageId } = request.params;
        const { myId } = request.body;
        const prisma = request.server.prisma;
...
const updateMessage = async (request, reply) => {
    try {
        ...
        const participantIds = members
            .map((m) => m.userId)
            .filter((id) => typeof id === "number");
        const transformed = (0, message_utils_1.transformMessage)(updated, participantIds);
        const response = {
            success: true,
        ...
        try {
            if (files.length > 0 && oldFiles.length) {
                fileService_1.FileService.removeFiles(oldFiles.map((f) => f.fileUrl).filter(Boolean));
            }
        }
        catch (_) { }
...
const markMultipleMessagesAsRead = async (request, reply) => {
    try {
...
const markMessageAsDelivered = async (request, reply) => {
    try {
        const { conversationId } = request.params;
        const { myId } = request.body;
        const prisma = request.server.prisma;
...
const getMessages = async (request, reply) => {
    try {
        ...
        const rows = allRows.filter((message) => {
            return !(0, jsonArray_1.jsonArrayContains)(message.deletedForUsers, myIdInt);
        });
        // Apply pagination after filtering
        const hasMore = rows.length > offset + perPage;
        const pageRows = rows.slice(offset, offset + perPage);
        const data = pageRows.map((m) => (0, message_utils_1.transformMessage)(m, participantIds));
        return reply.send({
            success: true,
            data,
            pagination: {
